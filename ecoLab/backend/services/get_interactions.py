...
def search_interactions(interactions: dict):
    ...
    try:
        result = ecoInteract.get_interaction_data(
...
def _build_species_interactor_map(
    interactions_list: list,
    selected_species_names: list,
) -> dict:
    """
    Returns:
        {
          interactor_name: {
              target_species_name: {interaction_types}
          }
        }

    An interactor shared by multiple target species will have an entry per target.
    If it is shared by ALL targets, the column will have values for every row (no NaN masking).
    """
    mapping = {}
    ...
    for interaction in interactions_list:
        ...
        for selected in selected_species_names:
            if source == selected and target and target not in selected_species_names:
                interactor = target
                mapping.setdefault(interactor, {}).setdefault(selected, set()).add(itype)
...
def add_interaction_occurrence(interactions: dict, use_year: bool = True):
    ...
    try:
        all_occ_df = ecoobs.get_occurrences(
            species_names=all_interactor_species,
            year_range=(year_min, year_max) if use_year and years else None,
            lat_min=lat_min,
            lat_max=lat_max,
            lon_min=lon_min,
            lon_max=lon_max,
    ...
    for interactor, target_types in interactor_map.items():
        all_types = set()
        for types in target_types.values():
            all_types |= types
        label = ", ".join(sorted(all_types))
        interactor_col_names[interactor] = f"{interactor} ({label})"

    species_col = None
    for candidate in ["species", "scientificName", "taxon", "name"]:
        if candidate in occurrences_df.columns:
    ...
    for interactor, target_types_map in interactor_map.items():
        ...
        species_occ = all_occ_df[
            all_occ_df["scientificName"].str.contains(interactor, case=False, na=False)
        ]
